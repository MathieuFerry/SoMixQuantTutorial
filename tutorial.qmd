...
It is coded as follows: modality 1 stands for "Currently attending school", 2 for "Never attended school" and 3 for "Attended school in the past". Yet, in the `edu.rds` database, we only have these modalities as number (1, 2, or 3) and not what they mean. In order to make our tables and graphs easier to read, we apply the appropriate labels to each value of this variable.
...
Similarly to what we did in the previous section on bivariate and multivariate statistics, we need to create a function that would combine the analysis of different variables:
...
School attendance is likely to be associated with a family’s social background for at least two reasons.
